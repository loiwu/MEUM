...
- (void)buildUI {
    ...
    [self cleanTabView];
    //build UI
    for (BAENTabItem *item in self.tabItems) {
...
- (void)swtichTab:(BAENTabItem *)tabItem {
...
- (void)pressedTab:(id)sender {
    ...
    BAENTabItem *tabItem = [self tabItemForTab:tabView];
    [self swtichTab:tabItem];
}

#pragma mark - Helper methods

- (UIControl *)existingTabForTabItem:(BAENTabItem *)tabItem {
    int index = [self indexOfTab:tabItem];
    if (self.tabViews.count > index) {//if (index != NSNotFound && self.tabViews.count > index)
...
- (CGRect)frameForTab:(BAENTabItem *)tabItem {
...
- (void)setTabContent:(UIControl *)tab withTabItem:(BAENTabItem *)tabItem {
...
//        titleLabel.adjustsLetterSpacingToFitWidth = YES;
    ...
    if (self.darkensBackgroundForEnabledTabs) {
        if (tabItem.tabState == TabStateEnabled) {
            [tab viewWithTag:DARKER_BACKGROUND_VIEW_TAG].backgroundColor = [UIColor colorWithWhite:1 alpha:0.15f];
        } else {
